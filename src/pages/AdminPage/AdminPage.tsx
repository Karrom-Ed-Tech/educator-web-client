    ...
    return (
        <div className="flex h-full mb-32">
            <div className="flex flex-col h-full justify-start p-3 bg-white w-full basis-1/4 p-page">
                        ...
                        <ul className="pt-2 pb-4 space-y-1 text-sm" >
                            {
                                sideNavItems.map((item:SideNavItem , index:number) => {
                                    return (
                                        <li className={`rounded-sm ${index === selectedItem ? "bg-secondary text-white px-3 py-2 rounded-xl" : ""}`}>
                                            <a
                                                href="#"
                                            ...
                                            >
                                                <span className="w-6 h-6">
                                                    <MaterialIcon codepoint={item.icon} className="text-primary"/>
                                                </span>
                ...
                {
                    selectedItem==0 ? <EducatorRequests/> : selectedItem==1 ? <ViewEducators/> : <Logs/>
                }
        </div>
    );
}